                ...
                <div class="modal-body text-sm">
                    <div class="form-group">
                        ...
                        @foreach($whatsapp_campaigns as $whatsapp_campaign)
                            <option value="{{ $whatsapp_campaign->id }}">{{ $whatsapp_campaign->name }}--{{$whatsapp_campaign->team_name}}--{{ $whatsapp_campaign->is_rm_name == 1 ? 'Rm Name In Template' : 'Lead Name In Template' }}</option>
                        @endforeach
                        </select>
                    </div>
                        <textarea class="form-control" id="phone_inp_id_m" name="phone_number_id_m" style="display: none"></textarea>
                </div>
...
<script>
    $("#sendMultiMessageBtn").click(function() {
        ...
        $.ajax({
            url: '{{ route('whatsapp_chat.create_task_by_number') }}',
            type: 'POST',
            data: data,
            dataType: 'json',
            success: function(response) {
                if (response.success) {
                    ...
                    camp_name.value = "";
                    var manageWhatsappChatModal = bootstrap.Modal.getInstance(document.getElementById('wa_msg_multiple'));
