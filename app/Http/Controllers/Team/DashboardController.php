...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $rm_unfollowed_leads = DB::table('leads')
            ...
            HAVING COUNT(CASE WHEN tasks.done_datetime IS NULL THEN 1 END) = 0) as completed_tasks"), 'completed_tasks.lead_id', '=', 'leads.lead_id')
        ...
        if ($auth_user->role_id == 5) {
            ...
            $task_schedule_this_month = Task::selectRaw('count(distinct(lead_id)) as count')
                ...
                ->where(['tasks.created_by' => $auth_user->id,  'lead_forwards.forward_to' => $auth_user->id])
            ...
            $unread_leads_this_month = Lead::where('lead_datetime', 'like', "%$current_month%")
                ...
                })->where('leads.deleted_at', null)->groupBy('lead_id')->get()->count();
