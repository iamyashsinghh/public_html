        ...
        $auth_user = Auth::guard('bdm')->user();
            ...
            if ($request->dashboard_filters != null) {
                ...
                if ($request->dashboard_filters == "leads_received_this_month") {
                    $leads->where('bdm_leads.lead_datetime', 'like', "%$current_month%")->whereNull('bdm_leads.deleted_at')->where('bdm_leads.forward_to', $auth_user->id);
                } elseif ($request->dashboard_filters == "leads_received_today") {
                    $leads->where('bdm_leads.lead_datetime', 'like', "%$current_date%")->whereNull('bdm_leads.deleted_at')->where('bdm_leads.forward_to', $auth_user->id);
                    ...
                    $leads->where('bdm_leads.lead_datetime', 'like', "%$current_month%")->whereNull('bdm_leads.deleted_at')->where(['bdm_leads.read_status' => false])->where('bdm_leads.forward_to', $auth_user->id);
                }elseif($request->dashboard_filters == "unread_leads_today"){
                    $leads->where('bdm_leads.lead_datetime', 'like', "%$current_date%")->where(['bdm_leads.read_status' => false])->whereNull('bdm_leads.deleted_at')->where('bdm_leads.forward_to', $auth_user->id);
                }elseif($request->dashboard_filters == "total_unread_leads_overdue"){
                    $leads->where('bdm_leads.lead_datetime', '>=', Carbon::parse('2024-02-01')->startOfDay())
                    ->where('bdm_leads.lead_datetime', '<=', Carbon::now())
                    ->where('bdm_leads.read_status', false)
                    ->whereNull('bdm_leads.deleted_at')
                    ->where('bdm_leads.forward_to', $auth_user->id)
                    ->distinct('bdm_leads.lead_id');
                }elseif($request->dashboard_filters == "forward_leads_this_month"){
                    $leads->join('nv_lead_forward_infos', 'bdm_leads.lead_id', '=', 'nv_lead_forward_infos.lead_id')
